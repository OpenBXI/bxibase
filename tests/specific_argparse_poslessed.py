# Original argparse unit test suite, played against posless
...
class TestDefaultSuppress(ParserTestCase):
    ...
    argument_signatures = [
        Sig('foo', nargs='?', default=posless.SUPPRESS),
        Sig('bar', nargs='*', default=posless.SUPPRESS),
        Sig('--baz', action='store_true', default=posless.SUPPRESS),
    ]
    failures = ['-x']
    successes = [
        ('', NS()),
...
class TestParserDefaultSuppress(ParserTestCase):
    ...
    parser_signature = Sig(argument_default=posless.SUPPRESS)
    argument_signatures = [
        Sig('foo', nargs='?'),
        Sig('bar', nargs='*'),
...
class TestFileTypeRepr(TestCase):
    ...
    def test_r(self):
        type = posless.FileType('r')
        self.assertEqual("FileType('r')", repr(type))

    def test_wb_1(self):
        type = posless.FileType('wb', 1)
...
class TestFileTypeR(TempDirMixin, ParserTestCase):
    ...
    argument_signatures = [
        Sig('-x', type=posless.FileType()),
        Sig('spam', type=posless.FileType('r')),
    ]
...
class TestFileTypeRB(TempDirMixin, ParserTestCase):
    ...
    argument_signatures = [
        Sig('-x', type=posless.FileType('rb')),
        Sig('spam', type=posless.FileType('rb')),
    ]
    failures = ['-x', '-x bar']
    successes = [
        ('foo', NS(x=None, spam=RFile('foo'))),
...
class TestFileTypeW(TempDirMixin, ParserTestCase):
    ...
    argument_signatures = [
        Sig('-x', type=posless.FileType('w')),
        Sig('spam', type=posless.FileType('w')),
    ]
    failures = ['-x', '-x bar']
...
class TestFileTypeWB(TempDirMixin, ParserTestCase):
    ...
    argument_signatures = [
        Sig('-x', type=posless.FileType('wb')),
        Sig('spam', type=posless.FileType('wb')),
    ]
    failures = ['-x', '-x bar']
    successes = [
        ('foo', NS(x=None, spam=WFile('foo'))),
...
class TestActionUserDefined(ParserTestCase):
    ...
    class OptionalAction(posless.Action):
    ...
    class PositionalAction(posless.Action):
...
class TestParentParsers(TestCase):
    ...
    def test_conflicting_parents(self):
        self.assertRaises(
            posless.ArgumentError,
            posless.ArgumentParser,
...
class TestMutuallyExclusiveGroupErrors(TestCase):
    ...
    def test_invalid_add_argument(self):
        ...
        raises(ValueError, add_argument, 'bar', nargs=posless.PARSER)
...
class TestMutuallyExclusiveFirstSuppressed(MEMixin, TestCase):
    ...
    def get_parser(self, required):
        parser = ErrorRaisingArgumentParser(prog='PROG')
        group = parser.add_mutually_exclusive_group(required=required)
        group.add_argument('-x', help=posless.SUPPRESS)
...
class TestMutuallyExclusiveManySuppressed(MEMixin, TestCase):
    ...
    def get_parser(self, required):
        ...
        add('--spam', action='store_true', help=posless.SUPPRESS)
        add('--badger', action='store_false', help=posless.SUPPRESS)
        add('--bladder', help=posless.SUPPRESS)
        return parser
...
class TestHelpSuppressUsage(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG', usage=posless.SUPPRESS)
    argument_signatures = [
        Sig('--foo', help='foo help'),
...
class TestHelpSuppressOptionalGroup(HelpTestCase):
    ...
    argument_group_signatures = [
        (Sig('group'), [Sig('--bar', help=posless.SUPPRESS)]),
    ]
    usage = '''\
...
class TestHelpSuppressPositional(HelpTestCase):
    ...
    argument_signatures = [
        Sig('--foo', help='foo help'),
        Sig('spam', help=posless.SUPPRESS),
    ]
    argument_group_signatures = []
    usage = '''\
...
class TestHelpRawText(HelpTestCase):
    ...
    parser_signature = Sig(
        prog='PROG', formatter_class=posless.RawTextHelpFormatter,
...
class TestHelpRawDescription(HelpTestCase):
    ...
    parser_signature = Sig(
        prog='PROG', formatter_class=posless.RawDescriptionHelpFormatter,
        description='Keep the formatting\n'
                    '    exactly as it is written\n'
                    '\n'
                    'here\n')
...
class TestHelpArgumentDefaults(HelpTestCase):
    ...
    parser_signature = Sig(
        prog='PROG', formatter_class=posless.ArgumentDefaultsHelpFormatter,
...
class TestInvalidArgumentConstructors(TestCase):
    ...
    def test_parsers_action_missing_params(self):
        ...
        self.assertTypeError('command', action='parsers',
                             parser_class=posless.ArgumentParser)
...
class TestConflictHandling(TestCase):
    ...
    def test_bad_type(self):
        self.assertRaises(ValueError, posless.ArgumentParser,
                          conflict_handler='foo')

    def test_conflict_error(self):
        parser = posless.ArgumentParser()
        parser.add_argument('-x')
        self.assertRaises(posless.ArgumentError,
                          parser.add_argument, '-x')
    ...
    def test_resolve_error(self):
        get_parser = posless.ArgumentParser
...
class TestStrings(TestCase):
    ...
    def test_optional(self):
        ...
        string = (
            ...
            "envvar=None, nargs='+', const=None, default=42, type='int', "
            "choices=[1, 2, 3], help='HELP', metavar='METAVAR')")
        self.assertStringEqual(option, string)

    def test_argument(self):
        argument = posless.Action(
            option_strings=[],
            dest='x',
            type=float,
            nargs='?',
...
class TestNamespace(TestCase):
    ...
    def test_constructor(self):
        ...
        ns = posless.Namespace(a=42, b='spam')
        self.assertEqual(ns.a, 42)
        self.assertEqual(ns.b, 'spam')

    def test_equality(self):
        ns1 = posless.Namespace(a=1, b=2)
        ns2 = posless.Namespace(b=2, a=1)
        ns3 = posless.Namespace(a=1)
        ns4 = posless.Namespace(b=2)
...
class TestEncoding(TestCase):
    ...
    def test_posless_module_encoding(self):
        self._test_module_encoding(posless.__file__)

    def test_test_posless_module_encoding(self):
...
class TestArgumentError(TestCase):
    ...
    def test_argument_error(self):
        msg = "my error here"
        error = posless.ArgumentError(None, msg)
...
class TestArgumentTypeError(TestCase):
    ...
    def test_argument_type_error(self):
        ...
        def spam(string):
            raise posless.ArgumentTypeError('spam!')
