...
node {
    ...
    env.DNAME = sh returnStdout: true, script: 'cat deps.make | cut -d " " -f 4 | rev | cut -c 3- | rev'
    env.DNAME = env.DNAME.trim().trim()
    ...
    sh '''
       ...
       scan-build -k -o .scanbuild -v make CFLAGS='-std=gnu99'
    ...
    stage('Package') {
    echo "Packaging.."
    env.BNAME = "${JOB_NAME}" - "/${BRANCH_NAME}"
    sh '''
     . $WORKSPACE/dependencies.sh install
    make devrpm
    cd $WORKSPACE
    tar -cf ${BNAME}.tar archives
    '''
    archiveArtifacts "${BNAME}.tar"
    }
